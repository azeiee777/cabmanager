...
<body class="bg-gray-950 text-gray-100 font-sans flex justify-center min-h-screen">
    ...
    <div class="w-full flex h-screen overflow-hidden bg-gray-950">
        ...
        <aside class="hidden md:flex flex-col w-64 bg-gray-900 border-r border-gray-800 z-30 shadow-2xl">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
                <a href="{{ route('dashboard.view') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('dashboard.view') ? 'bg-amber-500/10 text-amber-400 font-bold shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 font-medium' }}">
                    <i data-lucide="home" class="w-5 h-5"></i><span>Dashboard</span>
                </a>
                <a href="{{ route('ride.view') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('ride.view') ? 'bg-amber-500/10 text-amber-400 font-bold shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 font-medium' }}">
                    <i data-lucide="car" class="w-5 h-5"></i><span>Add Ride</span>
                </a>
                <a href="{{ route('expense.view') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('expense.view') ? 'bg-amber-500/10 text-amber-400 font-bold shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 font-medium' }}">
                    <i data-lucide="wallet" class="w-5 h-5"></i><span>Add Expense</span>
                </a>
                <a href="{{ route('tolls.view') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('tolls.view') ? 'bg-amber-500/10 text-amber-400 font-bold shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 font-medium' }}">
                    <i data-lucide="receipt" class="w-5 h-5"></i><span>Tolls Analysis</span>
                </a>
                <a href="{{ route('settings.view') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('settings.view') ? 'bg-amber-500/10 text-amber-400 font-bold shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 font-medium' }}">
            ...
            <div class="px-6 py-4 border-t border-gray-800 mt-auto bg-black/20">
                <div class="flex items-center gap-3">
                    <div
                        ...
                        <i data-lucide="user" class="text-amber-400 w-5 h-5"></i>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 font-medium">Logged in as</p>
                        <p class="text-sm font-bold text-gray-200 truncate w-32">{{ auth()->user()->identifier }}</p>
                    </div>
                </div>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col h-full relative overflow-hidden bg-gray-950 md:bg-gray-950/50">
            ...
            <!-- ===================================== -->
            <nav
                class="md:hidden absolute bottom-0 w-full bg-gray-900/90 backdrop-blur-md border-t border-gray-800 pb-safe z-30 pt-2 pb-4 px-2 flex justify-between items-center">
                <a href="{{ route('dashboard.view') }}"
                    class="flex-1 flex flex-col items-center transition-all {{ request()->routeIs('dashboard.view') ? 'text-amber-400 scale-110' : 'text-gray-500 hover:text-gray-300' }}">
                    <i data-lucide="home" class="w-6 h-6 mb-1"></i><span class="text-[10px] font-medium">Home</span>
                </a>
                <a href="{{ route('ride.view') }}"
                    class="flex-1 flex flex-col items-center transition-all {{ request()->routeIs('ride.view') ? 'text-amber-400 scale-110' : 'text-gray-500 hover:text-gray-300' }}">
                    <i data-lucide="car" class="w-6 h-6 mb-1"></i><span class="text-[10px] font-medium">Ride</span>
                </a>
                <a href="{{ route('expense.view') }}"
                    class="flex-1 flex flex-col items-center transition-all {{ request()->routeIs('expense.view') ? 'text-amber-400 scale-110' : 'text-gray-500 hover:text-gray-300' }}">
                    <i data-lucide="wallet" class="w-6 h-6 mb-1"></i><span
                        class="text-[10px] font-medium">Expense</span>
                </a>
                <a href="{{ route('tolls.view') }}"
                ...
                <a href="{{ route('settings.view') }}"
                    ...
                    <i data-lucide="settings" class="w-6 h-6 mb-1"></i><span
                        class="text-[10px] font-medium">Settings</span>
                </a>
            </nav>
        </div>
    </div>
    ...
    <script>
        ...
        $.ajaxSetup({
            ...
            error: function(jqXHR) {
                if (jqXHR.status === 401) window.location.href = "{{ route('login.view') }}";
            }
        });
        ...
        function showAlert(message, type = 'success') {
            ...
            if (type === 'success') {
                box.addClass('bg-emerald-500/10 border border-emerald-500/20 text-emerald-400');
            } else {
                box.addClass('bg-rose-500/10 border border-rose-500/20 text-rose-400');
            }
